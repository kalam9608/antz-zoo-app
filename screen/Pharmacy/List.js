...
const List = (props) => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      } else if (type_of == "Store") {
        LoadStoreData(1, searchModalText);
  ...
  const LoadStoreData = (count, search) => {
    let obj = {
      sort: sort,
      column: "name",
    };
    GetListZooStoreList(count, obj, search)
      .then((res) => {
        if (res?.success) {
          setTotalCount(res?.data?.total_count ?? 0);
          let dataArr = count == 1 ? [] : manufacturer;
  ...
  const LoadSupplierData = (count, search) => {
    let obj = {
      sort: sort,
      column: "label",
    };
    GetListZooSupplierList(count, obj, search)
      .then((res) => {
        if (res?.success) {
          // setTotalCount(res?.data?.total_count ?? 0);
          let dataArr = [];
          setManufacturer(dataArr.concat(res?.data));
          // if (res?.data?.total_count == 0) {
          //   setForAddManufacturer([{ active: "1", id: "0000", label: search }]);
          // }
        }
  ...
  const handleLoadMore = () => {
    if (
      ...
      if (type_of == "Manufacturer") {
        ...
        LoadStoreData(nextPage, searchModalText);
      }
    }
  };
                  ...
                  children={
                    <>
                      <View style={{ display: "flex", flexDirection: "row" }}>
                        ...
                        {/* <TouchableOpacity
                        onPress={() => {
                          navigation.navigate("EditMasterScreen", {
        ...
        >
          <FAB
            ...
            onPress={() => {
              if (type_of == "GST") {
                // AddMasterScreen
                // navigation.navigate("AddMasterScreen")
                navigation.navigate("AddMasterScreen", {
                  type_of: "GST",
                });
              }
